...
-  **总控 Agent 驱动**：由一个总控智能体统一调度 6 个**VS Code 原生 Skill**（`.github/skills/`），支持斜杠命令 `/` 快速调用，用户只需一句口语化指令即可触发全流程
...
| 能力 | 说明 |
|------|------|
| 🏗️ 原生Skill架构 | 6 个 VS Code 原生 Skill（`.github/skills/`），支持 `/` 斜杠命令 + 自动激活 + 渐进加载 |
...
| **核心能力** | 自动规划章节编号、子标题、预计篇幅，每张图表均附带完整文字设计方案（名称/类型/维度/内容/绘制工具）；单文件 Skill，无需渐进加载 |
...
| **核心能力** | 数据严格来自编程结果（不编造），公式 LaTeX 语法，变量符号与建模一致；已有图表直插 MD，缺失图表用【图表占位】标注；内置去 AI 味写作规范（6 条子规则），全程控制 AI 检测率；渐进加载：SKILL.md (~220行) + references/（ai-defense + citation-rules + writing-guide 按需） |
...
| **核心能力** | 双模式：① 设计模式输出文字方案（名称/类型/维度/配色/数据来源/设计说明）② 绘制模式——数据图用 Python matplotlib（含 3D projection='3d'，中文标注，紧凑布局，DPI≥300），逻辑流程图用 draw.io（生成 .drawio 文件，VS Code 双击编辑）；渐进加载：SKILL.md (~150行) + references/（matplotlib-guide + drawio-guide 按需） |
...
graph TB
    subgraph 准备阶段
        ...
        A2 --> B[lit-review 阅读范文提取风格]
    end

    subgraph 写作阶段
        C[outline-planner 生成大纲] --> D[paper-writer 逐章写作]
        E[chart-designer 图表设计+绘制]
        E --> E1[数据图 matplotlib]
        E --> E2[流程图 draw.io]
    end

    subgraph 评审阶段
        F[paper-reviewer 评委模拟打分]
    end

    subgraph 输出阶段
        G[latex-builder 格式合规检查]
        G --> H[MD转LaTeX填充]
        H --> I[XeLaTeX 编译]
        I --> J{编译成功}
        J -->|是| K[最终 PDF]
...
│   │   └── 图标模板/             # 图表配色与撞色设计
│   └── 评委记忆/                 # 历史评审意见
├── 技能核心库/                   # 📦 旧版 Skill 备份（参考保留，Agent 不再从此处读取）
│   ├── 01-文献阅读与整理 SKILL.md
│   ├── 02-论文大纲规划 SKILL.md
│   ├── 03-论文写作 SKILL.md
│   ├── 04-图表设计 SKILL.md
