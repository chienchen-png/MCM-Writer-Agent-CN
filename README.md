...
> 6 大 Skill · MinerU PDF 识别 · draw.io 流程图 · matplotlib 3D 绘图 · CNKI 知网集成 · LaTeX 自动排版
...
| 图表设计与绘制 | 设计+绘制双模式：Python matplotlib 数据图（含 3D）+ draw.io 逻辑流程图 |
| 论文评审 | 双模式：格式质检 + 国赛评委模拟打分 |
| LaTeX 排版 | 模板填充 → 格式合规检查 → XeLaTeX 编译 → 自动修复 |
| 智能绘图 | 数据图自动生成（matplotlib）+ 流程图自动生成（draw.io），含 3D 可视化 |
...
| **核心能力** | 数据严格来自编程结果（不编造），公式 LaTeX 语法，变量符号与建模一致；已有图表直插 MD，缺失图表用【图表占位】标注（团队建模手绘制） |
